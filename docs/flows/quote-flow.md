...
- [app/quote/[id]/email/page.tsx](../../app/quote/[id]/email/page.tsx) + `EmailComposer.tsx` — compose and send the quote via a chosen `email_templates` row; actual send goes through [app/api/send-quote/route.ts](../../app/api/send-quote/route.ts), which independently recalculates, saves a **new** `quotes` row, and emails via Resend
  - **"Why we're suited" blurbs:** a template can contain `{{why_suited:<name>}}`, resolved inline by `EmailComposer` from the named `why_suited_templates` row (managed in Settings → Why we're suited). The **Generate custom why we're suited** button on the composer builds a ChatGPT prompt (wording in `why_suited_settings.prompt`) from that blurb + the event's `raw_email`, for Victoria to paste into ChatGPT and hand-edit the result back in. `send-quote` builds its own HTML and does **not** touch `email_templates`, so this placeholder only matters for the copy-to-clipboard path.
...
| `why_suited_templates` / `why_suited_settings` | read-only here (email composer); written by Settings → Why we're suited (`/api/admin/why-suited`) |
