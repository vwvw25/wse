...
- **`logEventActivity` is mandatory, not optional**, for any write to a table scoped to an event. This is [ADR-011](../decisions/ADR-011-event-activity-log.md)'s rule, and every flow doc above calls it out at the relevant point — the Activity tab on an event is supposed to be a complete audit trail, and it silently isn't if a new mutation forgets this call. ADR-011 also carries the **standing question to ask on every feature change** — "should this be in the activity feed, and how does this change affect it?" — and the four-places checklist for adding a new `EventActivityType`.
