...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: slot, error: slotErr } = await supabase
      ...
      .select('*, musician:musicians(*), event:events(*, booked_template:band_templates!booked_band_template_id(name))')
      .eq('id', slotId)
      .single()
    ...
    // Create a musician_invite row — token is auto-generated by the DB
    const { data: invite, error: inviteErr } = await supabase
      .from('musician_invites')
      .insert({
        slot_id: slotId,
        musician_id: slot.musician_id,
    ...
    if (inviteErr || !invite) {
      console.error('Failed to create musician_invite:', inviteErr)
    ...
    // Update the invite row with send result
    await supabase
      .from('musician_invites')
      .update({
